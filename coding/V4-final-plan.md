...
│   ├── handoff.md                  # Session 接力摘要（当前进度 + 下一步 + 关键上下文）
...
全流程产物量大，不可能全部加载到 LLM 上下文中。每个阶段只加载当前相关的 artifacts：
...
**原则**：每个阶段的输入是上一阶段的 artifacts，不是完整对话上下文。re-anchor 机制从 EXECUTE 扩展到所有阶段——每进入新阶段都重新锚定。
...
| 涉及不可逆的技术选型（如主语言 / 主框架） | must_confirm | 成本极高，需人工确认 |
